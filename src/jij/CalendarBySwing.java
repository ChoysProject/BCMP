...
public class CalendarBySwing extends JFrame implements WindowListener, ActionListener, ItemListener{
	...
	public CalendarBySwing(Customer co){
		cust = co;
...
//		this.setDefaultCloseOperation(EXIT_ON_CLOSE);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		// year를 EmplSystem의 두번째 콤보박스에(jcos[1])
		cust.jcos[0].setSelectedItem(Integer.parseInt(year));
		// month를 EmplSystem의 세번째 콤보박스에(jcos[2])
		cust.jcos[1].setSelectedItem(Integer.parseInt(month));
		// day를 EmplSystem의 네번째 콤보박스에(jcos[3])
		cust.jcos[2].removeAllItems();
		switch(Integer.parseInt(month)) {
		case 2: 
			for(int d = 1; d <= 28; d++) {
				cust.jcos[2].addItem(d);
			}
			break;
		case 4: 
		case 6: 
		...
		cust.jcos[2].setSelectedItem(Integer.parseInt(day));
		dispose();
	}
