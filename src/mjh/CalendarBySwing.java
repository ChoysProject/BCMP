...
public class CalendarBySwing extends JFrame implements WindowListener, ActionListener, ItemListener{
	...
	private ReturnSystem rs;
	public CalendarBySwing(ReturnSystem rs) {
		this.rs=rs;
...
//		System.out.println(year+","+month+","+day);
		//year를 EmplSystem의 두번째 콤보박스(choices[1])
		rs.box[1].setSelectedItem(Integer.parseInt(year));
		//month를 EmplSystem의 세번째 콤보박스(choices[2])
		rs.box[2].setSelectedItem(Integer.parseInt(month));
		//day를 EmplSystem의 네번째 콤보박스(choices[3]);
		rs.box[3].removeAllItems();
		switch(Integer.parseInt(month)) {
		case 2:for(int d=1;d<=28;d++) {
			rs.box[3].addItem(d);
		}break;
		case 4:
		case 6:
		case 9:
		case 11:for(int d=1;d<=30;d++) {
			rs.box[3].addItem(d);
		}break;
		default:for(int d=1;d<=31;d++) {
			rs.box[3].addItem(d);
		}break;
		
	 }
		rs.box[3].setSelectedItem(Integer.parseInt(day));
	}
